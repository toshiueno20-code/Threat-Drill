...
Threat Drillは、Gemini 3の高度な推論能力を活用した、次世代AIエージェント専用のセキュリティプラットフォームです。従来のセキュリティツールが単発のリクエストを監視するのに対し、Threat Drillは**100万トークン超のコンテキスト窓**をフル活用し、マルチステップ攻撃や画像・音声に埋め込まれた悪意のあるコマンドを検出します。
...
```bash
git clone https://github.com/your-org/Threat-Drill.git
cd Threat-Drill
```
...
curl -X POST https://YOUR-SERVICE-URL/api/v1/static-analysis/scan \
  ...
  -d '{
    ...
    "create_pr": true,
    "pr_branch_name": "threatdrill/security-fixes"
  }'
```
...
```bash
# Cloud Loggingでログを表示
gcloud logging read "resource.type=cloud_run_revision AND resource.labels.service_name=threatdrill-gatekeeper" --limit 50
```
...
- **Issue Tracker**: [GitHub Issues](https://github.com/your-org/Threat-Drill/issues)
- **Documentation**: [Wiki](https://github.com/your-org/Threat-Drill/wiki)
